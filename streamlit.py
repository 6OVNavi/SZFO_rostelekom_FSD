...
def prepare(uploaded_file):
    ...
    graph = pd.DataFrame(
        {'label_amount': gr.subject.value_counts()[:5], 'subject': gr.subject.value_counts().keys()[:5]}).reset_index(
        drop=True)
    return table, gr, graph
...
def main(uploaded_file):
    ...
    table, gr, graph = prepare(uploaded_file)
    if result:
    ...
    if mapa=='С регионами' and not gra:
        ...
        map=region(to_map)
        st.write(map)
    if mapa=='С метками'and not gra:
        to_map = pd.DataFrame({'amount_connect': list(gr.subject.value_counts()),
                                   'subject': list(gr.subject.value_counts().keys())})
        to_map = to_map.sort_values(by='subject')
        map=label(table, to_map)
        st.write(map)
